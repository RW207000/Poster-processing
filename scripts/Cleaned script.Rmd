...
totaldailyformedswide <- medperpatient %>%
  select(assessment_id, total_standardised_daily_dose_1)
...
# add in BMI
BMI_values <- select(longitudinal, c("Assessment.ID", "BMI"))
BMI_values <- rename(BMI_values, "assessment_id" = "Assessment.ID")

medperpatient <- left_join(medperpatient, BMI_values, by = "assessment_id")
...
CAH_biomarkers <- select(labs_wide, c("Assessment.ID", "CO.ID", "Date", "17-OHP.value", "17-OHP.interpretation", "Androstenedione.value", "Androstenedione.interpretation", "Renin.value", "Renin.interpretation"))
...
CAH_biomarkers$Renin.interpretation <- (str_extract(CAH_biomarkers$Renin.value, "[aA-zZ]+.?[aA-zZ]+.?[aA-zZ]*+"))
...
CAH_biomarkers_order <- c("Assessment.ID", "CO.ID", "Date", "17-OHP.value", "17-OHP.unit", "17-OHP.interpretation", "Androstenedione.value", "Androstenedione.unit", "Androstenedione.interpretation", "Renin.value", "Renin.interpretation", "Renin.plasma.activity")
...
blood_pressures$Class [blood_pressures$BMI <18.5] <- "Underweight"
BMIframe$Class [BMIframe$BMI >= 18.5 & BMIframe$BMI <25] <- "Normal"
